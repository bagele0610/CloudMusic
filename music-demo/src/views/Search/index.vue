...
// 2.找接口，api/Search.js里获取搜索关键词的请求方法
...
// 目标：加载更多
// 1.集成list组件 - 定义相关变量(搞懂变量的作用) - 检测触底事件
// 2.一旦触底，自动执行onload方法
// 3.对page++，给后台传递offset便宜量参数 - 请求下一页的数据
// 4.把当前数据和下一页新来的数据拼接起来用在当前页面的数组里
// (切记) -- 记载更多数据后，一定要把loading改为false，保证下一次还能出发onload方法
...
export default {
  data() {
    return {
      ...
      hotArr: [], //热搜关键字
      resultList: [], //搜索结果
      loading: false, //加载中(状态) - 只有为false，才能出发后自动触发onload方法
  ...
  methods: {
    ...
    async inputFn() {
      ...
      if (this.value.length === 0) {
        //搜索关键词如果没有，九八搜索结果清空阻止网络请求发送(提前return)
        this.resultList = [];
        return;
      }
      const res = await this.getListFn();
    ...
    async onLoad() {
      ...
      if (res.data.result.songs ===  undefined) {
        //证明没有数据
        this.finished = true; //只要设置true，list再次触发都不会执行onLoad函数(显示"没有更多数据了")
        this.loading = false;
        return;
      }
      this.resultList = [...this.resultList, ...res.data.result.songs];
      this.loading = false; //数据加载完毕 - 保证下一次还能出发onload
    }
  },
  components:{
    SongItem
